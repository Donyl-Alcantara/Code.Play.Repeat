...
class Scene:
    ...
    def _update_typewriter(self, current_time: int) -> None:
        if not self.typewriter_complete:
            if self.is_final_scene and self.text == "On-Screen Controls:\nPlayers tap or click on a face-down card to flip it over.\nYou can only flip two cards at a time" and not self.space_pressed:
...
class OpeningSequence:
   def __init__(self):
       ...
       pygame.display.set_icon(pygame.image.load("first_farm.png"))
   ...
   def _initialize_scenes(self) -> list:
        return [
            Scene("first_farm.png", 
                ...
                self.font, 
                "first_naturesound.wav",
                is_first_scene=True),
            
            Scene("second_ritual.png", 
                ...
                self.font, 
                "second_ritualsong.wav"),
            
            Scene("third_panic.png",
